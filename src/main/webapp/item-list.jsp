...
<html>
<head>
    <title>Pahana Edu - Item List</title>
    ...
    <div class="main-content">
        ...
        <div class="search-form-container" style="margin-bottom: 20px;">
            <form action="items" method="get">
                <input type="hidden" name="action" value="list"> <%-- Servlet එකේ list action එකට යවනවා --%>
                <div class="form-group" style="display: flex; align-items: center; gap: 10px;">
                    <input type="text" id="search" name="search" placeholder="Search by Name or Item ID" value="${searchTerm}" style="flex-grow: 1; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                    <button type="submit" class="btn btn-primary">Search</button>
                    <a href="items?action=list" class="btn btn-secondary">Clear</a>
                </div>
            </form>
        </div>

        <p>
            <a href="items?action=new" class="btn btn-success">Add New Item</a>
        </p>
        ...
        <table border="1">
            ...
            <tbody>
            <c:forEach var="item" items="${listItem}"> <%-- ItemServlet එකෙන් එන "listItem" attribute එක --%>
                <tr>
                    <td>${item.itemId}</td>
                    <td>${item.itemName}</td>
                    ...
                    <td>
                        <a href="items?action=view&itemId=${item.itemId}" class="btn btn-info btn-sm">View</a>
                        <a href="items?action=edit&itemId=${item.itemId}" class="btn btn-primary btn-sm">Edit</a>
                        <a href="items?action=delete&itemId=${item.itemId}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete item ${item.itemName} (${item.itemId})?');">Delete</a>
                    </td>
                </tr>
            </c:forEach>
