...
<html>
<head>
    <title>Pahana Edu - <c:if test="${item != null}">Edit</c:if><c:if test="${item == null}">Add</c:if> Item</title>
...
<div class="container">
    <div class="main-content">
        <h2><c:if test="${item != null}">Edit</c:if><c:if test="${item == null}">Add New</c:if> Item</h2>
        ...
        <form action="items" method="post">
            ...
            <input type="hidden" name="action" value="<c:if test="${item != null}">update</c:if><c:if test="${item == null}">insert</c:if>">

            <%-- Hidden field for original item ID when updating --%>
            <c:if test="${item != null}">
                <input type="hidden" name="originalItemId" value="<c:out value="${item.itemId}"/>">
            </c:if>

            <div class="form-group">
                <label for="itemId">Item ID:</label>
                <input type="text" id="itemId" name="itemId" value="<c:out value="${item.itemId}"/>" <c:if test="${item != null}">readonly</c:if> required>
                <c:if test="${item != null}"><small> (Item ID cannot be changed)</small></c:if>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="number" id="unitPrice" name="unitPrice" value="<c:out value="${item.unitPrice}"/>" step="0.01" min="0" required>
            </div>
            <div class="form-group">
                <label for="quantityInStock">Quantity in Stock:</label>
                <input type="number" id="quantityInStock" name="quantityInStock" value="<c:out value="${item.quantityInStock}"/>" min="0" required>
            </div>
            <button type="submit" class="btn btn-primary">
                <c:if test="${item != null}">Update</c:if><c:if test="${item == null}">Add</c:if> Item
            </button>
            <a href="items?action=list" class="btn btn-secondary">Cancel</a>
        </form>
    </div>
</div>
