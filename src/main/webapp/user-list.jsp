...
<div class="container">
    <div class="main-content">
        ...
        <c:if test="${not empty sessionScope.errorMessage}"> <%-- Error from deleting own account --%>
            <div class="message error">${sessionScope.errorMessage}</div>
            <c:remove var="errorMessage" scope="session"/>
        </c:if>

        <p>
            <a href="user?action=registerForm" class="btn btn-success">Register New User</a>
        </p>
        ...
        <table border="1">
            ...
            <tbody>
            <c:forEach var="user" items="${listUsers}"> <%-- UserServlet එකෙන් එන "listUsers" attribute එක --%>
                <tr>
                    <td>${user.id}</td>
                    <td>${user.username}</td>
                    <td>${user.role}</td>
                    <td>
                        <a href="user?action=editUserForm&id=${user.id}" class="btn btn-primary btn-sm">Edit</a>
                        <a href="user?action=deleteUser&id=${user.id}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete user ${user.username} (ID: ${user.id})?');">Delete</a>
                    </td>
                </tr>
            </c:forEach>
