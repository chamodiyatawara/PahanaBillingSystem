...
<head>
    ...
    <style>
        /* Specific styles for user form page */
        .user-form-container {
        ...
        /* New form field and button layout styles */
        ...
        @media (max-width: 480px) {
            .user-form-container { width: 92%; padding: 20px; margin: 30px auto; }
        ...
        <nav class="main-nav" role="navigation" aria-label="Main Navigation">
            ...
            <ul id="nav-menu" class="nav-list">
                ...
                <c:if test="${sessionScope.currentUser.role == 'admin'}">
                    ...
                    <li><a href="user?action=listUsers" class="nav-link active">Manage Users</a></li>
                </c:if>
            </ul>
        </nav>
...
<div class="user-form-container">
    ...
    <form action="user" method="post">
        ...
        <input type="hidden" name="id" value="${user.id}"> <%-- User ID එක hidden field එකක් විදියට යවනවා --%>
        ...
        <div class="form-group">
            ...
            <input type="password" id="password" name="password" value="<c:out value="${user.password}"/>" required> <%-- Password එකත් පෙන්වනවා --%>
            <small> (Enter new password to change, or keep current)</small>
        </div>
        <div class="form-group">
            <label for="role">Role:</label>
            <select id="role" name="role" required>
                <option value="cashier" <c:if test="${user.role == 'cashier'}">selected</c:if>>Cashier</option>
                <option value="admin" <c:if test="${user.role == 'admin'}">selected</c:if>>Admin</option>
            </select>
        </div>
        <div class="form-actions">
            <button type="submit" class="btn btn-primary">Update User</button>
            <a href="user?action=listUsers" class="btn btn-secondary" role="button">Cancel</a>
        </div>
    </form>
</div>
